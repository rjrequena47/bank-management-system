...
public class TransactionService {
        ...
        public TransactionResponse deposit(UUID accountId, UUID customerId, DepositRequest request) {
                ...
                Account account = accountRepository.findById(accountId)
                                .orElseThrow(() -> {
                                        log.warn("[TransactionService] Intento de depósito en cuenta inexistente. accountId={}",
                                                        accountId);
                ...
                if (account.getStatus() != AccountStatus.ACTIVE) {
                        log.warn("[TransactionService] Depósito denegado: cuenta inactiva. accountId={}, status={}",
                                        accountId,
                                        account.getStatus());
        ...
        public TransactionResponse withdraw(UUID accountId, UUID customerId, WithdrawalRequest request) {
                ...
                Account account = accountRepository.findById(accountId)
                                .orElseThrow(() -> {
                                        log.warn("[TransactionService] Intento de retiro en cuenta inexistente. accountId={}",
                                                        accountId);
                                        return new AccountNotFoundException("Cuenta no encontrada");
                                });
                ...
                if (account.getStatus() != AccountStatus.ACTIVE) {
                        log.warn("[TransactionService] Retiro denegado: cuenta inactiva. accountId={}, status={}",
                                        accountId,
                                        account.getStatus());
        ...
        public Page<TransactionResponse> getTransactionsByAccount(
                        ...
                        Pageable pageable) {
                log.info("[TransactionService] Consultando transacciones. accountId={}, customerId={}", accountId,
                                customerId);
                ...
                Page<Transaction> transactionsPage = transactionRepository.findByAccountIdAndFilters(accountId,
